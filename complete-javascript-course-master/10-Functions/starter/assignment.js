...
const poll = {
  ...
  registerNewAnswer() {
    ...
    const userVote = prompt(`${this.question} \n\
                                  ${this.options[0]} \n\
                                  ${this.options[1]} \n\
                                  ${this.options[2]} \n\
                                  ${this.options[3]}`);
    ...
    //typeof answer === 'number' && answer < this.answer.length && this.answer[answer]++ ---Teacher solution

    if (Number(userVote) >= 0 && Number(userVote) <= 3) {
      //Add to answeres
      let valueOfPos = Number(this.answers[userVote]);
      valueOfPos++;
      this.answers[userVote] = valueOfPos;
      // console.log(this.answers);
    } else {
      console.log(
        `Answer "${userVote}" was not a valid answer. It has to be from 0 to 3 to be a valid vote.`
      );
    }
    ...
    // Me
    return displayResults(this.answers);
  },
  //Teachers solution to displayresult case
...
const displayResults = function (type = []) {
  if (typeof type === typeof []) {
    ...
    console.log(`Poll results are ${this.type}`);
  }
};
